  ...
  methods: {
    cargad_Datos(url) {
      axios.get(url).then((response) => {
        ...
        this.tarjetas_debito = this.tarjetas.filter((tarjeta) =>
          tarjeta.type.includes("DEBITO")
        );
        this.tarjetas_credito = this.tarjetas.filter((tarjeta) =>
          tarjeta.type.includes("CREDITO")
        );
      });
    },
    salir: function () {
    ...
    Crear_Cuenta: function () {
      axios
        .post("/api/cliente/current/cuenta")
      ...
      axios
        .post(
          ...
          "monto=" +
            ...
            this.cuentaOrigen +
            "&numeroDeDeDestino=" +
            this.cuentaDeDestino
        )
        .then((response) => alert("Transacion Finalisada Con exito"));
    },
  },
}).mount("#app");
